...
class Nav2Navigator(Node):
    ...
    def go_to_pose(
        self, x: float, y: float, yaw: float, frame_id: str = "map", wait: bool = True
    ):
        """
        ...
        if not self._action_client.server_is_ready():
            self.get_logger().error(
                "navigate_to_pose server not ready; call wait_for_server() first"
            )
            return None
...
Examples:
  ...
  # Use different frame
  ros2 run steve_navigation nav2_navigator.py --x 1.0 --y 1.0 --yaw 0.0 --frame odom
    """,
    )
    ...
    parser.add_argument(
        "--frame", type=str, default="map", help="Reference frame (default: map)"
    )
    parser.add_argument(
        "--no-wait",
        action="store_true",
    ...
    navigator.go_to_pose(
        x=args.x, y=args.y, yaw=args.yaw, frame_id=args.frame, wait=not args.no_wait
    )
